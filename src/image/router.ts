...
router.get('/:id/info.json', async ctx => {
    if (ctx.params.id === 'logo' || ctx.params.id === 'audio') {
        logger.info(`Received a request for image info of the ${ctx.params.id}`);

        if (ctx.params.id === 'logo' && !config.logoRelativePath)
            throw new HttpError(404, 'No logo');
        if (ctx.params.id === 'audio' && !config.audioRelativePath)
            throw new HttpError(404, 'No audio');

        setContentType(ctx);
        ctx.body = await getStaticImageInfo(ctx.params.id, getProfile());

        logger.info(`Sending image info of the ${ctx.params.id}`);
    }
    else {
...
router.get('/:id/:region/:size/:rotation/:quality.:format', async (ctx: ImageContext) => {
    if (ctx.params.id === 'logo' || ctx.params.id === 'audio') {
        logger.info(`Received a request for the ${ctx.params.id}`);
        ...
        const image = ctx.params.id === 'logo'
        ...
        logger.info(`Sending the ${ctx.params.id}`);
    }
    else {
        const [id, tier] = ctx.params.id.split(config.imageTierSeparator);

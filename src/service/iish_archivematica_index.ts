...
import {FileItem, FolderItem, Item} from '../lib/ItemInterfaces';
...
export default async function processDip({collectionPath}: IndexParams): Promise<void> {
    ...
    try {
        ...
        runTask<TextParams>('text', {collectionId: rootItem.id, items: texts});
    }
    catch (e) {
...
function getRootItem(mets: Document): Item {
...
function walkTree({id, mets, objects, relativeRootPath, curNode, curNodePhysical, structureIISH, parent = null}:
    ...
    (curNode.find('./mets:div', ns) as Element[]).forEach(node => {
        ...
        if (!nodePhysical)
            throw new Error(`Could not find a div in the physical structmap with the label "${label}"`);
...
function readFolder(rootId: string, mets: Document, node: Element, nodePhysical: Element, parent: string | null)
    : Item | null {
...
function readFile(rootId: string, mets: Document, objects: string[], relativeRootPath: string,
                  node: Element, nodePhysical: Element, structureIISH: Element | null, parent: string | null): Item | null {
    const labelAttr = nodePhysical.attr('LABEL');
    const label = labelAttr ? labelAttr.value() : null;
    if (!label)
        throw new Error(`Expected to find a file with a label`);

    const fptrElem = nodePhysical.get('mets:fptr', ns);
...
function readText(rootId: string, mets: Document, objects: string[], relativeRootPath: string,
                  node: Element, nodePhysical: Element, structureIISH: Element, parent: string): TextItem | null {
    const labelAttr = nodePhysical.attr('LABEL');
    if (!labelAttr)
        throw new Error(`Expected to find a file with a label}`);
    ...
    const fptrElem = nodePhysical.get('mets:fptr', ns);
    if (!fptrElem || !fptrElem.attr('FILEID'))
        throw new Error(`Missing a fptr or file id for a file with the label ${labelAttr.value()}`);
    ...
    if (premisObj) {
        ...
        if (fptr) {
            if (!fptr.attr('FILEID'))
                throw new Error(`Missing a file id for a file with the label ${labelAttr.value()}`);
...
function getIdentifier(premisObj: Element, premisNS: 'premis' | 'premisv3' = 'premis') {
...
function determineResolution(objCharacteristicsExt: Element): { width: number | null; height: number | null } {
...
function determineDpi(objCharacteristicsExt: Element): number | null {
    const exifTool = objCharacteristicsExt.get('./rdf:RDF/rdf:Description', ns);
    if (exifTool) {
...
function determineDuration(objCharacteristicsExt: Element): number | null {
